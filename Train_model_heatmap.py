...
#         return loss
    ...
    def train_val_sample(self, sample, n_iter=0, train=False):
        ...
        self.conf_thresh = torch.tensor(0.015)
        
        heatmap_org = heatmap_org.double()
        self.conf_thresh = self.conf_thresh.double()
        heatmap_orig = torch.where(heatmap_org >= self.conf_thresh, heatmap_org, 0.0) # Confidence threshold.
        
        heatmap_warp = heatmap_warp.double()
        heatmap_warp = torch.where(heatmap_warp >= self.conf_thresh, heatmap_warp, 0.0) # Confidence threshold.
...
#         )
        ...
        _, _ = torch.sort(heatmap_orig, dim = 1, descending=True)
        
        heatmap_warp = heatmap_warp.view(heatmap_warp.shape[0],-1)
        _, _ = torch.sort(heatmap_warp, dim = 1, descending=True)
                                  
                                  
        loss_heatmap = self.heatmap_loss_weight * (torch.pow(heatmap_orig - heatmap_warp, 2)).mean()
