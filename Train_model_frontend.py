...
class Train_model_frontend(object):
    ...
    def get_loss(self, semi, labels3D_in_loss, mask_3D_flattened, device="cpu"):
        ...
        self.gamma = 2
        self.alpha = 0.25
        ce_loss = loss_func(semi, labels3D_in_loss)
        pt = torch.exp(-ce_loss)
        focal_loss = self.alpha * ((1 - pt) ** self.gamma * ce_loss)
        loss = (focal_loss * mask_3D_flattened).sum()
    ...
    def train_val_sample(self, sample, n_iter=0, train=False):
        ...
        Hc = int(H / self.cell)
        Wc = int(W / self.cell)
        semi_tran = semi.transpose(1, 2, 0)
        heatmap_orig = np.reshape(semi_tran, [Hc, Wc, self.cell, self.cell])
        heatmap_orig = np.transpose(heatmap_orig, [0, 2, 1, 3])
        heatmap_orig = np.reshape(heatmap_orig, [Hc*self.cell, Wc*self.cell])
        heatmap_orig = np.where(heatmap_orig >= self.conf_thresh, heatmap_orig, 0) # Confidence threshold.
        
        semi_warp_tran = semi_warp.transpose(1, 2, 0)
        heatmap_warp = np.reshape(semi_warp_tran, [Hc, Wc, self.cell, self.cell])
        heatmap_warp = np.transpose(heatmap_warp, [0, 2, 1, 3])
        heatmap_warp = np.where(heatmap_warp >= self.conf_thresh, heatmap_warp, 0) # Confidence threshold.
        
        heatmap_warp = nn.functional.interpolate(
            heatmap_warp, scale_factor=(self.cell, self.cell), mode="bilinear"
        )
        
        self.heatmap_loss_weight = 0.0001
        loss_heatmap = self.heatmap_loss_weight * (torch.pow(torch.mm(mat_H,heatmap_orig) - torch.mm(heatmap_warp, img), 2)).mean()
        ...
        loss = (
            loss_heatmap + loss_det + loss_det_warp + self.config["model"]["lambda_loss"] * loss_desc
        )
